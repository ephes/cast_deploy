...
async def authenticate_user(db, username: str, password: str):
    user = await crud.aget_user_by_name(db, username)
    if not user:
        return False
...
async def get_current_user(db: Connection, token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        raise credentials_exception
    #user = await crud.aget_user_by_name(db, username=token_data.username)
    user = None
    if user is None:
        raise credentials_exception
    return user
